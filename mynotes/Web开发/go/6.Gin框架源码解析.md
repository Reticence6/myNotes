...
https://liwenzhou.com/posts/Go/gin-sourcecode/
...
httprouter：是 Go 生态中最经典、性能最强的**底层路由库**，也是 **Gin、Echo、Fiber 等主流框架高性能路由的 “鼻祖”**。**纯路由（mux）**，不是 Web 框架。**纯路由（mux）**，不是 Web 框架。零内存分配、O (k) 匹配、极省内存、无正则、无反射。

Gin **没有直接依赖** httprouter，而是**参考设计、自己重写**了 Radix Tree 逻辑，在路由之上加了 **gin.Context、中间件链、路由分组、绑定 / 验证、渲染** 等全套 Web 功能（HttpRouter：**只有路由分发**，无中间件、无 Context）
...
![image-20260413112439321](./assets/image-20260413112439321.png)
...
由于URL路径具有层次结构，并且只使用有限的一组字符(字节值)，所以很可能有许多常见的前缀。这使我们可以很容易地将路由简化为更小的问题。此外，**路由器为每种请求方法管理一棵单独的树（Get、Post等分别为一棵树）**。一方面，它比在每个节点中都保存一个method-> handle map更加节省空间，它还使我们甚至可以在开始在前缀树中查找之前大大减少路由问题。
...
1. 为什么 Gin 能用占位符（/user/:id）匹配？

   - 在 [**Gin**](chatgpt://generic-entity?number=0) 的 **Radix Tree** 里，节点不仅存字符串，还存 **节点类型**。

   - Gin 的节点类型大致有三种：

     | **类型** | **示例**      | **作用**   |
     | -------- | ------------- | ---------- |
     | static   | /user/login   | 普通字符串 |
     | param    | /user/:id     | 参数节点   |
     | catchAll | /static/*file | 通配节点   |

   - /user/:id在树里不会存成字符串 :id，而是存成：param node，匹配逻辑是：如果当前节点是 param，就匹配任意字符串直到下一个 /。所以**占位符匹配其实是节点类型控制的，而不是字符串匹配。**

     | **类型** | **写法**  | **匹配范围**            |
     | -------- | --------- | ----------------------- |
     | 参数节点 | :id       | 只匹配一个 path segment |
     | 通配节点 | *filepath | 匹配后面所有路径        |
...
2. 为什么每个 Method 要一棵树？
   ...
   - 优点 1：超级省空间
     ...
     - 每棵树只存**对应方法的路由**
     - 节点里**不需要存 method → handler map**
     - 节点结构变小，树更干净

   - 优点 2：查找速度直接起飞（最关键！）

     - Gin 做的第一步：**直接选中 GET 那棵树，别的树直接不看！**
     - 等于：**在查找前缀树之前，就把路由数量减少了 4~8 倍！**
     - 先按方法筛选 → 再查树 → 查找规模直接缩小几倍！
   ...
   - 优点 3：路径可以重复！
     ...
     - 如果共用一棵树，必须靠 map 区分方法。

### 源码解析

1. *context对象 --> 对象池

   - **对象池**就是：提前创建一批对象，循环复用，而不是每次都重新创建。
   - 优点：减少内存分配，减少GC（垃圾回收）压力，提高吞吐量（显著减少了alloc和GC的时间）
   ...
   func (engine *Engine) ServeHTTP(w http.ResponseWriter, req *http.Request) {
     ...
     // 从池子取Context，如果对象池有直接拿，没有就调用new创建
     ...
     // 设置当前的http请求
   	c.Request = req
   	// 清空旧数据（params、handlers、errors、keys）
   	c.reset()
   
     // 处理请求（路由匹配、中间件执行、handler执行）
...
2. `for i, tl := 0, len(t); i < tl; i++ `

   - 将len赋值给tl，这样的话len只需要计算一次就可以，不需要多次计算
   - 写成 i < len(t) 时，循环条件在每轮都会重新求值；但 len 对字符串、切片这类类型通常非常便宜，而且编译器也常会优化，所以把 len 存到 tl 更多是一种性能风格和明确意图的写法。

3. 由2引申：普通for和range的区别

   - 普通for访问的是原始元素，range中的v是元素副本
   - 每次判断len，如果修改了长度，for循环行为会变，但是range不会
   - 

   | **对比维度**           | **普通** for                | range                    |
   | ---------------------- | --------------------------- | ------------------------ |
   | 基本形式               | for i := 0; i < len(s); i++ | for i, v := range s      |
   | 控制能力               | 完全手动控制循环逻辑        | 语言自动展开遍历         |
   | 是否产生元素副本       | 不会，直接访问 s[i]         | 会产生副本 v := s[i]     |
   | 修改元素               | 可以直接修改 s[i]           | 修改 v 不影响原数据      |
   | 获取元素地址           | &s[i] 是真实地址            | &v 是循环变量地址        |
   | 遍历长度               | 每轮判断 len(s)             | 通常在开始时确定遍历范围 |
   | 是否能动态改变遍历长度 | 可以（如果 len 变了）       | 一般不会改变遍历次数     |
   | 循环变量行为           | 每次迭代变量是新的          | 循环变量会被复用         |
   | 性能可控性             | 更高，可避免拷贝            | 更简洁但可能有隐式拷贝   |
   | 可读性                 | 稍微复杂                    | 更简洁                   |
   | 常见用途               | 性能敏感代码、底层框架      | 普通遍历                 |

   - 不同数据类型下的行为

   - slice/array

     - range数组会涉及拷贝问题：Go 中 `range` 遍历数组（**数组本身是值类型**）时，会完整拷贝整个数组作为迭代源；而遍历**切片（Slice）**或**数组指针**时，不会拷贝数组，只会拷贝切片头 / 指针，性能完全不同。

       - 优化1：遍历数组切片 `arr[:]`（最常用）

         ```go
         arr := [1000000]int{...}
         // 把数组转成切片，仅拷贝切片头（3个字段，24字节，64位系统），不拷贝数组
         for _, v := range arr[:] {
             fmt.Println(v)
         }
         ```
       ...
       - 优化2：遍历数组指针 `&arr`
         ...
         ```go
         arr := [1000000]int{...}
         // 遍历数组指针，仅拷贝指针（8字节，64位系统），不拷贝数组
         for _, v := range &arr {
             fmt.Println(v)
         }
         ```

       - 补充：Go 1.22 对 `range` 做了优化：

         - 支持 `range` 数组指针 `&arr`，迭代时直接访问原数组，无拷贝
         - 编译器会自动优化「仅用索引」的 `range arr`，但**不保证所有场景**，仍建议手动转切片
   ...
   - string
     ...
     - Go字符串底层是UTF-8字节序列，普通for会一个字节一个字节读，range按rune（Unicode码点）读
     - 求字符串长度：

     ```go
     // 错误
     fmt.Println(len("世界")) // 输出 6 (因为每个中文占3字节)
     // 正确
     fmt.Println(len([]rune("世界"))) // 输出 2 (得到真实的字符个数)
     ```
   ...
   - channel
     ...
     - range：只要通道没关，就继续接收，通道关闭后自动结束循环。
   ...
   - range中的v是循环变量（本身是一个被反复复用的变量，地址每次可能都是相同的）

4. `var _ IRouter = (*Engine)(nil)`

   -  这是Go 里一种**编译期接口断言**写法，用来确保 *Engine 实现了 IRouter 接口。
   - **(\*Engine)(nil)**：这是一个 ***Engine 类型的 nil 指针**。意思不是创建对象，而是构造一个“类型是 *Engine” 的值。
   - 编译器会检查：*Engine 是否实现了 IRouter 接口。如果实现了，编译通过。如果没实现，编译直接报错。
   - **为什么要写成** ***Engine**，而不是 **Engine**
     - 因为 Go 判断“某个类型是否实现接口”时，**值类型**和**指针类型**是分开的。
   - **为什么要用nil**
     - 因为这里只是为了做**类型检查**，不需要真的分配对象。
     - 所以写：`(*Engine)(nil) `比写：`&Engine{}`更轻量，也更常见。
   - 作用：
     - 防止接口实现丢失
     - 让源码读者一眼看懂关系
...
![gin_middleware1](https://liwenzhou.com/images/Go/gin/gin_middleware1.png)

我们可以在中间件函数中通过再次调用`c.Next()`实现嵌套调用（func1中调用func2；func2中调用func3），

![gin_middleware2](https://liwenzhou.com/images/Go/gin/gin_middleware2.png)
...
`c.Set()`和`c.Get()`这两个方法多用于在多个函数之间通过`c`传递数据的，比如我们可以在认证中间件中获取当前请求的相关信息（userID等）通过`c.Set()`存入`c`，然后在后续处理业务逻辑的函数中通过`c.Get()`来获取当前请求的用户。`c`就像是一根绳子，将该次请求相关的所有的函数都串起来了。![gin_middleware3](https://liwenzhou.com/images/Go/gin/gin_middleware3.png)
